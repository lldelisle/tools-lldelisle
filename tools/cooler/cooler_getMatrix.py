...
def getHeaders(c,r,a):
  ...
  return([ "%s|%s|%s:%i-%i"%(i,a,c[0],s,e) for i,c,s,e in zip(fragID,chromID,starts,ends) ])

def extractMatrix(args):
  c = cooler.Cooler(args.input)
  mat=c.matrix(balance=args.balance).fetch(args.r1,args.r2)
  if args.header:
    ass=c.info['genome-assembly']
    rowH=getHeaders(c,args.r1,ass)
    colH=getHeaders(c,args.r2,ass)
...
argp.add_argument('--header',action="store_true", help='If you want to print row names and col names (by default there are only values).')
