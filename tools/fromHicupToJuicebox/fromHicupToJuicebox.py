    ...
    for i, line in enumerate(f):
      ...
      if(len(v)<max(colC, colS, colE, colI)):
        raise InputError(len(v)<max(colC, colS, colE, colI),
                         "The ids of columns specified in the input for the fragment file are incompatible with the line :"+line)
...
def readSamFromHicupAndWriteOutputForJuicebox(in_samOrBam, fo,useMid, bigDic, method):
  """"return the validpair file <readname> <str1> <chr1> <pos1> <frag1> <str2> <chr2> <pos2> <frag2> <mapq1> <mapq2> 
  str = strand (0 for forward, anything else for reverse) 
  pos = 5'of the read (position used to find the fragment in the 2 restriction enzyme mode 
  but not for the sonication protocol where the position used to find the fragment is 10 bp downstream 
  the most upstream coordinate of the mapped positions (the one given by bowtie)).
