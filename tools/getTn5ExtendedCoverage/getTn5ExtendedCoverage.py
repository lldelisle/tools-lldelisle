...
def reportCoverage(chrName,starts,ends,fileout,lengthChrom):
  ...
  for i in range(minindex,maxindex):
    ...
    #The coverage changed
      if (lastDepth>0):
        ...
        fileout.write(chrName+"\t"+str(lastStart)+"\t"+str(i)+"\t"+str(lastDepth)+"\n")
  ...
  #Print the last
  if (lastDepth>0):
    fileout.write(chrName+"\t"+str(lastStart)+"\t"+str(maxindex+1)+"\t"+str(lastDepth)+"\n")
...
argp=argparse.ArgumentParser(description="compute coverage like bedtools of 5' of reads shifted 4 or 5 bases and extended of length.")
argp.add_argument('--input',default=None,help="input coordinates-sorted bam with alignement.")
