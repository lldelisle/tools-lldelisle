...
def writeResultsFromOv(ImagePlus imp, Integer quantif_ch, Integer t, Overlay ov,
                       String now, String tool_version, Integer n_pieces,
                       Integer roi_gastruloid_id, String roi_gastruloid_name,
...
def processImage(Client user_client, ImageWrapper image_wpr,
    ...
    for (roi_i = 0; roi_i < omeroRois.size(); roi_i ++) {
        ...
        if (roi_name.toLowerCase().startsWith("gastruloid_")) {
            ...
            if (potential_id.contains('_id')) {
                ...
                gastruloid_roi_ids.put(potential_id, omeroRW[roi_i])
            } else {
                gastruloid_rois.put("t" + current_t, roi)
                gastruloid_roi_ids.put("t" + current_t, omeroRW[roi_i])
            }
        }
        if (roi_name.toLowerCase().startsWith("spine")) {
            ...
            if (split_res.length > 1) {
                gastruloid_rois.put(split_res[1], roi)
            } else {
                gastruloid_rois.put("t" + current_t, roi)
            }
    ...
    omeroRW.each{
        ...
        if ((roi_name.startsWith("WholeGastruloid_")) ||
            (roi_name == "Full_") ||
            (roi_name == "NonGastruloid_") ||
            (roi_name == "WholeBackground_") ||
    ...
    for (int t = 1; t <= nT; t++ ){
        ...
        writeResultsFromOv(imp, quantif_ch, t, ov,
    ...
    for (String id in gastruloid_rois.keySet())
        ...
        if (spine_roi != null) {
            ...
            writeResultsFromOv(imp, quantif_ch, t, ov,
                               now, tool_version, n_pieces,
                               gastruloid_roi_ids.get(id), gastruloid_roi.getName(),
                               image_wpr, id)
        }
    }
...
String tool_version = "Fluo_v20230730"
